...
export default function App() {
  ...
  return (
    <div className="site-shell">
      ...
      {view === "resources" && (
        <ResourcesView config={config} items={content.resources} />
      )}
    ...
    <>
      <section className="home-hero">
        <div className="hero-inner home-hero-grid">
          <div>
            <span className="eyebrow">{config.heroEyebrow}</span>

            <h1>
              {config.heroTitle}
              <br />
              <em>{config.heroAccent}</em>
            </h1>
    ...
    <>
      ...
      <section className="content-section compact">
        ...
        <div className="case-count">
          <b>{filtered.length}</b>개의 품질관리 사례
        </div>
              ...
              >
                <div className="case-visual">
                  {item.before_image_path ? (
                    <img
                      src={publicAssetUrl(item.before_image_path)}
                      alt="사례 사진"
                    />
                  ) : (
                    <i>!</i>
...
function ManualView({
  ...
  const filtered = items.filter(
    (item) =>
      item.faq === faq &&
      `${item.title} ${item.summary} ${item.body}`.includes(query),
  );
...
function ResourcesView({
  ...
  const filtered = items.filter(
    (item) =>
      (category === "전체" || item.category === category) &&
      `${item.title} ${item.description} ${item.file_name}`.includes(query),
  );
